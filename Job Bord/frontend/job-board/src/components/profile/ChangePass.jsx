...
const ChangePass = () => {
  ...
  useEffect(() => {
    const checkPassExist = async () => {
      try {
        const response = await axiosInstance.get('/auth/check-pass-set', { params: { user_id }, headers: { authorization: 'Bearer ' + token } })
      ...
      <div className="flex flex-col w-5/6 bg-white ml-auto mr-auto mt-4 gap-4 p-5 border-2 border-gray-400">
        ...
        <div className=" ">
          <div className='flex h-12 bg-gray-300 justify-center items-center '>
            <label className="text-center text-lg text-red-800 italic font-semibold">{isPassSet === true ? 'Change' : 'Set'} Password </label>
          </div>
